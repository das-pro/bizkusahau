...
class Ion_auth_model extends CI_Model {
    ...
    public function update_onlineusers($data,$userid){
        $check = $this->db->get_where('users_online',array('user_id'=>$userid))->row();
        if($check){
            return $this->db->update('users_online',$data,array('user_id'=>$userid));
        }else{
            return $this->db->insert('users_online',$data);
        }
    }
    ...
    public function login_remembered_user() {
        ...
        if ($query->num_rows() == 1) {
            ...
            $update_onlineuser = array(
                ...
                'user_id'=>$user->id,
                'lasttime'=>  time());
            $this->update_onlineusers($update_onlineuser, $user->id);
            $this->set_session($user);
